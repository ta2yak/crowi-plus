...
module.exports = function(crowi, app) {
  ...
  if (env === 'production' && helpers.hasProcessFlag('onpremise')) {
    const fs = require('graceful-fs');
    const logfilePath = path.join(crowi.logsDir, 'express.log');
    stream = fs.createWriteStream(logfilePath);
  }
  const options = require('../util/logging/config-express')(env);
  app.use(pino(options, stream));
