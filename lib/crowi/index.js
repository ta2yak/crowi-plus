...
function Crowi (rootdir, env)
{
  ...
  this.logsDir      = path.join(this.rootDir, 'logs') + sep;
  ...
  this.searcher = null;
  this.mailer = {};
  this.pinoRootLogger = undefined;  // will be initialized in setupLogger()
...
Crowi.prototype.setupLogger = function() {
  const pinoDebug = require('pino-debug')
      , pino = require('pino')
      , pinoOpts = require('../util/logging/config-app')(this.node_env)
      , pinoDebugMappings = require('../util/logging/pino-debug-mappings')(this.node_env)
      ;

  // determine stream
  let stream = process.stdout;
  // production on-premise
  if (this.node_env === 'production' && helpers.hasProcessFlag('onpremise')) {
  ...
  return new Promise((resolve, reject) => {
    this.pinoRootLogger = pino(pinoOpts, stream);
    this.pinoRootLogger.info('logger is setup');

    // NOTE: pino-debug can only be called once.
    pinoDebug(this.pinoRootLogger, {
      map: pinoDebugMappings
    });
